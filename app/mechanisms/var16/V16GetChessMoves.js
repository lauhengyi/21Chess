import V16CheckPin from "./functions/V16CheckPin.js";
import V16CheckCheck from "./functions/V16CheckCheck.js";
import getPiece from "../primaryFunctions/getPiece.js";
import V16MovePiece from "./functions/V16MovePiece.js";
...
function V16GetChessMoves(piece, board, occupiedMatrix, lastMoved, type) {
  switch (type) {
    case "moves":
...
function normalMoves(piece, board, occupiedMatrix) {
  ...
  for (let move of movesUnchecked) {
    if (V16CheckPin(move, board) === false) {
      moves.push(move);
    }
  }
  return moves;
...
function checkEnPassant(piece, board, lastMoved) {
  ...
  let newBoard = V16MovePiece([lastMoved[0], ghostPosition], board);
...
function checkCastling(piece, board, occupiedMatrix, validMoves) {
  ...
  if (V16CheckCheck(board, occupiedMatrix, piece.side)) {
    return castleMoves;
  }
  ...
  for (let validMove of validMoves) {
    ...
    if (validMove[1] === piece.position - 1 && castlingRookLeft[0]) {
      ...
      if (
        ...
        const newBoard = V16MovePiece(validMove, board);
        const newPiece = getPiece(piece.id, newBoard);
        const newOccupiedMatrix = getOccupiedMatrix(newBoard);
...
export default V16GetChessMoves;

import V13CheckPin from "./functions/V13CheckPin";
import V13CheckCheck from "./functions/V13CheckCheck.js";
...
import V13CreatePieceDataCalculator from "./V13CreatePieceDataCalculator.js";
import getOccupiedMatrix from "../primaryFunctions/getOccupiedMatrix.js";

function V13GetChessMoves(piece, board, occupiedMatrix, lastMoved, type) {
  switch (type) {
    case "moves":
...
function validAttacks(piece, occupiedMatrix) {
  // Get moveable moves
  let pieceData = V13CreatePieceDataCalculator(piece, occupiedMatrix);
...
function normalMoves(piece, board, occupiedMatrix) {
  ...
  for (let move of movesUnchecked) {
    if (V13CheckPin(move, board) === false) {
      moves.push(move);
    }
  }
  return moves;
...
function checkCastling(piece, board, occupiedMatrix, validMoves) {
  ...
  if (V13CheckCheck(board, occupiedMatrix, piece.side)) {
    return castleMoves;
  }
...
export default V13GetChessMoves;

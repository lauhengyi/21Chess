...
function V8ChessMovesReducer(state, action) {
  //Making deep copy
  ...
  switch (action.type) {
    case "pieceClick": {
      ...
      if (state.capturingPieces.includes(action.pieceId)) {
        return state;
      }
    ...
    case "makeTurn": {
      ...
      //Make move
      //Prevent moves that move previously capturing pieces
      if (!state.capturingPieces.includes(action.move[0])) {
        ...
        const statusNum = side ? 1 : 2;
        return {
          boardLayout: state.boardLayout,
          checkmated: statusNum,
          stalemated: 0,
          checK: 0,
          repetition: 0,
        };
      ...
      //Change Side (only if no promotion and non capturing move
      if (action.move.length > 2) {
        //Add capturing piece
        newDetails.capturingPieces.push(action.move[0]);
      } else if (!newDetails.promotion) {
        newDetails.currentSide = !state.currentSide;
        //Remove capturing piece
        newDetails.capturingPieces = [];
      }
...
export default V8ChessMovesReducer;

...
function V4GetChessMoves(piece, board, occupiedMatrix, lastMoved, type) {
  switch (type) {
    case "moves":
...
function normalMoves(piece, board, occupiedMatrix) {
  ...
  for (let move of movesUnchecked) {
    if (V4CheckPin(move, board) === false) {
      moves.push(move);
    }
  }
  return moves;
...
function checkEnPassant(piece, board, lastMoved) {
  ...
  let newBoard = V4MovePiece([lastMoved[0], ghostPosition], board);
  ...
  for (let validMove of validMoves) {
    ...
    if (validMove[1] === piece.position - 1 && castlingRookLeft[0]) {
      ...
      if (
        ...
        const newBoard = V4MovePiece(validMove, board);
        const newPiece = getPiece(piece.id, newBoard);
        const newOccupiedMatrix = getOccupiedMatrix(newBoard);
...
export default V4GetChessMoves;

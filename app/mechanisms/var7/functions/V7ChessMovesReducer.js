...
import initialiseTurns from "./initialiseTurns.js";
import getTurn from "./getTurn.js";

function V7ChessMovesReducer(state, action) {
  //Making deep copy
  ...
  switch (action.type) {
    ...
    case "makeTurn": {
      ...
      if (!newDetails.promotion) {
        newDetails.currentSide = newDetails.turnsOrder[0];
        //Remove first turns
        newDetails.turnsOrder.splice(0, 1);
        //Add next first turns
        newDetails.turnsOrder.push(
          getTurn(newDetails.turnsOrder[newDetails.turnsOrder.length - 1])
        );
      }
...
export default V7ChessMovesReducer;

import V14CheckPin from "./functions/V14CheckPin";
import V14CheckCheck from "./functions/V14CheckCheck.js";
import getPiece from "../primaryFunctions/getPiece.js";
import movePiece from "../var0/functions/movePiece";
import checkCollision from "../var0/functions/checkCollision.js";
import V14CreatePieceDataCalculator from "./V14CreatePieceDataCalculator.js";
import "react-native-console-time-polyfill";
import getOccupiedMatrix from "../primaryFunctions/getOccupiedMatrix";

function V14GetChessMoves(piece, board, occupiedMatrix, lastMoved, type) {
  switch (type) {
    case "moves":
...
function validAttacks(piece, occupiedMatrix) {
  // Get moveable moves
  let pieceData = V14CreatePieceDataCalculator(piece, occupiedMatrix);
...
function normalMoves(piece, board, occupiedMatrix) {
  ...
  for (let move of movesUnchecked) {
    if (V14CheckPin(move, board) === false) {
      moves.push(move);
    }
  }
  return moves;
...
function checkCastling(piece, board, occupiedMatrix, validMoves) {
  ...
  if (V14CheckCheck(board, occupiedMatrix, piece.side)) {
    return castleMoves;
  }
...
export default V14GetChessMoves;

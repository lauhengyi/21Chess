...
function V14EvaluateBoardV2(gameDetails, getChessMoves) {
  ...
  function getBaseValue(piece) {
    if (piece.stacked && piece.stacked !== "x") {
      //Search of redundent merges
      //qp, qr, qb
      if (piece.type === "q") {
        if (piece.stacked !== "n") {
          return queenValue;
        } else {
          return queenValue + knightValue * 1.2;
        }
      } else if (piece.stacked === "q") {
        if (piece.type !== "n") {
          return queenValue;
        } else {
          return queenValue + knightValue * 1.2;
      ...
      //Multiply the more powerful piece by 1.1
      const value1 = getSimpleValue(piece.type);
      const value2 = getSimpleValue(piece.stacked);
      if (value1 > value2) {
        return value1 * 1.1 + value2;
      } else {
        return value2 * 1.1 + value1;
      }
    } else {
      return getSimpleValue(piece.type);
    }
  }

  function getSimpleValue(type) {
    switch (type) {
      case "p":
        return pawnValue;
      case "r":
...
export default V14EvaluateBoardV2;

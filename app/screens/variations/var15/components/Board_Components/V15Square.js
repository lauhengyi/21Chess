...
function getStyle(color, isMoveableOnSquare, isClicked, isOrderOnSquare) {
  ...
  const squareColor =
    isMoveableOnSquare || isOrderOnSquare ? color[1] : color[0];
...
function checkSquare(gameDetails, position) {
  ...
  const orders = gameDetails.orders;
  ...
  let moveableMove;
  //Check orders
  let isOrderOnSquare = false;
  let orderOnSquare = null;
  for (let order of orders) {
    if (order[0] === position) {
      isOrderOnSquare = true;
      orderOnSquare = order;
      break;
    }
  }
